...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (oldMessage.author.id !== '270904126974590976')
        console.log(`old message (${oldMessage.guild.name}):\n${oldMessage}\nnew message:\n${newMessage}`);
...
client.on('messageCreate', async (message) => {
    if (message.channel.id === '894742416345665576' &&
        message.author.id !== '696554549418262548') {
        if (message.content.toLowerCase() === 'science' ||
            message.content.toLowerCase().startsWith('bio')) {
            message.member?.roles
                .add(message.guild.roles.cache.find((role) => role.name === 'verified'))
                .then(() => {
                message.delete();
            });
        }
    ...
    if (args.shift().toLowerCase() === 'clear') {
        ...
        if (isNaN(parseInt(args[0]))) {
            message.reply({
                embeds: [
                    {
                        title: 'the amount of messages you want me to clear must be a valid number below 100',
                        color: 'RED',
                    },
                ],
            });
                ...
                embeds: [
                    {
                        ...
                        description: "the discord api doesn't let me. Number of messages must be below 100.",
                        color: 'RED',
                    },
                ],
            });
                ...
                embeds: [
                    {
                        ...
                        description: 'Why +1? becuase you sent a command. that needs to be deleted.',
                        color: 'GREEN',
                    },
                ],
            })
    ...
    if (message.content.toLowerCase().startsWith('pls snipe')) {
            ...
            ? {
                embeds: [
                    new discord_js_1.MessageEmbed()
                        .setDescription(`${message.author.bot
        ...
        await message.reply(snipe
            ? {
                embeds: [
                    new discord_js_1.MessageEmbed()
                        .addField('Old message:', snipe.content)
                        .addField('New message:', `[Jump!](https://discord.com/channels/${message.guild.id}/${message.channel.id}/${snipe.id})`)
                        .setAuthor(snipe.author.tag)
                        .setColor('RANDOM')
                        .setFooter(`#${channel.name}`)
                        .setTimestamp(snipe.createdAt ? snipe.createdAt : 0),
